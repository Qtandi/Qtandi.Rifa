<div>
    ...
    <div class="flex items-center justify-center justify-items-center pt-3">
        <p wire:model.lazy="price" class="text-2xl font-bold text-[#6FF0B2] me-3">R{{ Number::currency($this->price) }}</p>
        <div class="relative flex items-center max-w-full">
            ...
            <input wire:ref="quantity" name="quantity" min="20" max="5000" wire:input="updatePrice" wire:model="quantity" type="number" id="quantity-input" data-input-counter class="bg-gray-50 border-x-0 border-gray-100 border-2 text-center text-sm block text-[#ACABAB] w-full py-2.5 font-bold" value="{{ $quantity }}" required />
        ...
        <button wire:click="finalize" class="shadow-sm ms-3 text-[#D08852] bg-[#F4A060] hover:text-white hover:bg-[#C3814F] focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
